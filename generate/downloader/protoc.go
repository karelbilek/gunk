...
func (pd Protoc) CheckOrDownload(path, version string) (string, error) {
	if version == "" {
	...
	if os.IsExist(err) {
		...
		if err := pd.verifyProtocBinary(dstPath, version); err != nil {
			return "", err
		}
		return dstPath, nil
	}
	...
	url, err := pd.protocDownloadURL(runtime.GOOS, runtime.GOARCH, version)
	if err != nil {
...
func (Protoc) verifyProtocBinary(path, version string) error {
...
func (Protoc) protocDownloadURL(os, arch, version string) (string, error) {
